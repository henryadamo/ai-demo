...
original_author="henryadamo"
original_name="ai_demo"
original_urlname="ai-demo"
original_description="Awesome ai_demo created by henryadamo" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/ai_demo "${name}"
...
then
    ...
    echo "then run 'ai_demo' or 'python -m ai_demo'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'ai_demo' or 'python -m ai_demo' or 'poetry run ai_demo'"
fi
